...
func Voer(plan cli.Plan) error {
	...
	case cli.ModusSamengevoegd:
		...
		return tekenSamengevoegd(plan.Taken[0])
	case cli.ModusLos:
...
func tekenTaak(taak cli.Taak) error {
...
func tekenSamengevoegd(taak cli.Taak) error {
	document := pdfout.Nieuw()
	vel := document.Vel()
	for i, bron := range taak.Bronnen {
		...
		if err := render.Teken(blokken, vel); err != nil {
			return err
		}
	}
	if err := document.Schrijf(taak.Doel); err != nil {
